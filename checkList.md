...
- [x] **`쿠폰 관리` 화면 신규 개발** (2026-09-20): ADMIN 전용 `/coupons` 생성·목록·사용자 ID 직접 발급 및 메뉴/접근 제한 구현. `80fd615`, `caf9c4e` 커밋을 `origin/master`에 푸시 완료(배포 확인은 별도).
  ...
  - 타입 검사 및 모의 API E2E 5개 통과: 생성 요청·목록 갱신, 발급 실패/성공, 활성 토글, 조회 실패/재시도, OWNER 접근 차단. ESLint는 설정 파일 부재로 실행 불가
...
- [x] **쿠폰 총 발급 한도 보장** (2026-09-21, `f0fd569`): 관리자 직접 발급과 프로모 코드 등록이 공통 `assertIssuable()`을 거치도록 통합. 한도는 `usedCount`(실제 사용 수)가 아니라 발급된 `UserCoupon` 수 기준으로 검사하고, 목록 API는 `_count`로 `issuedCount`를 함께 반환한다. 동시 요청이 겹치면 1~2장 초과할 수 있으나 예산 통제 목적에는 충분하다고 보고 별도 잠금은 두지 않음(서비스 주석에 근거 명시)
- [x] **쿠폰 중복 오류 안내** (2026-09-21, `f0fd569`·`69db9cd`): 프로모 코드 중복 생성은 사전 조회 후 `이미 사용 중인 프로모 코드입니다`, 동일 고객 재발급은 `이미 보유한 쿠폰입니다`, 한도 초과는 `발급 한도가 초과된 쿠폰입니다`로 각각 400 응답. DB 유일성 위반으로 500이 나가던 경로 제거
- [x] **쿠폰 실 API 연동 검증** (2026-09-21, 운영 환경): 실 ADMIN 계정으로 `admin.tacomole.kr/coupons`에서 정액 3,000원·최소 15,000원·한도 2장·프로모 코드 `VERIFY0921` 쿠폰 생성(`POST /coupons` 201). 중복 코드 재생성 400(`이미 사용 중인 프로모 코드입니다`), `PATCH /coupons/:id/active` 200 및 비활성 시 발급 버튼 차단 확인. 배달앱에서 프로모 코드 등록 → 마이페이지 노출 → 재등록 차단 → 결제 화면에서 상품 금액 기준 3,000원 할인(배달비 유지) 확인. 관리자 목록의 `발급` 수가 0장 → 1장으로 증가해 `issuedCount`가 실 DB 기준임을 확인. 검증용 쿠폰은 비활성 처리(삭제 API 없음)
  - 미검증: 정률 쿠폰은 상한 입력 필드 노출까지만 확인하고 실제 생성은 하지 않음. 총 발급 한도 초과(3장째) 차단은 고객 계정이 하나 더 필요해 남음
- [x] **쿠폰 활성 토글** (2026-09-21, `69db9cd`): `PATCH /coupons/:couponId/active` 추가 및 관리자 화면 활성화/비활성화 버튼 연결. 비활성 쿠폰은 발급 버튼이 비활성화된다. 같은 커밋에서 카드 표기도 `발급 n장 / 한도 m장`과 `사용 n장`으로 분리해, 발급 수와 사용 수를 한 칸에 섞어 쓰던 오해 소지를 제거
...
- [x] **김포점 배달 메뉴 등록** (2026-09-21): 네이버 플레이스 메뉴판 기준으로 공개 33건 등록. 기존 5건은 `대표 메뉴` → 실제 카테고리(Tacos·Quesadilla·Saiad)로 이동했다. `Menu`에 `isFeatured`가 없어 '대표'를 카테고리로 두면 같은 메뉴가 `전체` 탭에 두 번 나오므로, 메뉴는 실제 카테고리 한 곳에만 두고 `대표 메뉴`는 비웠다(프론트에서 자동 숨김)
  - 제외: `[한그릇 할인 전용]`은 배민 메인화면 전용 할인가라 자사앱에 넣지 않았다
  - 검증: 배달앱 카테고리 탭 9개 + `전체`, 신규 메뉴 렌더 확인. `대표 메뉴`·`테스트 카테고리`는 숨겨짐
- [ ] **김포점 메뉴 이미지 등록**: 33건 전부 `imageUrl`이 비어 있어 배달앱에 회색 자리만 보인다. 배달앱은 이미지 영향이 커서 오픈 전 필수
- [x] **메뉴 옵션 그룹 등록** (2026-09-21): 관리자 화면에서 직접 등록. `Jarritos 하리토스` → `맛 선택`(필수, 1개: 망고·파인애플·구아바·자몽·라임), `Soda 탄산음료` → `종류 선택`(필수, 1개: 콜라·스프라이트·환타파인·환타오렌지). 전부 추가금 0원
  - 배달앱 메뉴 상세에서 선택지가 실제로 뜨는 것까지 확인. 그 전에는 설명 텍스트로만 있어 고객이 고를 수 없었고 주문이 와도 무슨 맛인지 알 수 없었다
- [ ] **메뉴 가격 배달가 점검**: 네이버(홀) 표기가 그대로 등록했다. 배달가가 다르면 수정 필요
...
- [x] **웹 토스트 UI** (2026-09-21): `lib/webToast.ts`(모듈 스코프 pub/sub) + `components/ui/ToastHost.tsx`를 추가하고 `showToast`가 웹에서 이를 쓰도록 했다. React 밖(쿼리 콜백·훅)에서도 호출 가능하고, 네이티브 Toast처럼 한 번에 하나만 표시한다. `useFavorites`는 로그인 화면 이동 대신 토스트 안내로 되돌렸다. E2E 1건 추가, delivery-customer 36건 통과
  - 전역 쿼리 에러 안내(`providers.tsx`)도 이제 웹에서 실제로 보인다. 그동안 웹에서는 조용히 무시됐다
