...
type AdminCoupon = Coupon & { maxUses: number | null; usedCount: number; issuedCount: number };
...
const initialForm = {
  ...
  description: '',
  code: '',
  type: 'FIXED_AMOUNT' as Coupon['type'],
  discountValue: '',
...
const buttonClass =
  'rounded-md border border-gray-300 px-3 py-2 text-sm font-medium hover:bg-gray-50 disabled:opacity-50';
const primaryClass =
  'rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50';
...
function formatDiscount(coupon: AdminCoupon) {
  ...
  const cap = coupon.maxDiscountAmount == null
    ? '상한 미설정'
    : `최대 ${formatNumber(coupon.maxDiscountAmount)}원`;
  return `${formatNumber(coupon.discountValue)}% 할인 (${cap})`;
}
...
export default function CouponsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      setFeedback({ type: 'error', message: getHttpErrorMessage(error, '쿠폰 생성에 실패했습니다.') });
    },
  });
  ...
  const issueMutation = useMutation({
    mutationFn: async ({ couponId, recipient, days }: { couponId: string; recipient: string; days: string }) =>
      adminApi.post(
        `${API_URL}/coupons/${couponId}/issue`,
    ...
    onError: (error) => {
      setFeedback({ type: 'error', message: getHttpErrorMessage(error, '쿠폰 발급에 실패했습니다.') });
    },
  });

  const activeMutation = useMutation({
    mutationFn: async ({ couponId, isActive }: { couponId: string; isActive: boolean }) =>
      adminApi.patch(`${API_URL}/coupons/${couponId}/active`, { isActive }, { headers: authHeaders }),
    onSuccess: (_result, variables) => {
      setFeedback({
        type: 'success',
        message: variables.isActive ? '쿠폰을 활성화했습니다.' : '쿠폰을 비활성화했습니다.',
      });
      invalidate();
    },
    onError: (error) => {
      setFeedback({ type: 'error', message: getHttpErrorMessage(error, '쿠폰 상태 변경에 실패했습니다.') });
    },
  });
  ...
  function submitIssue(event: FormEvent<HTMLFormElement>, coupon: AdminCoupon) {
    event.preventDefault();
  ...
  function openIssueForm(couponId: string) {
  ...
  async function copyCode(code: string) {
    try {
      ...
      setFeedback({
        type: 'error',
        message: '복사하지 못했습니다. 프로모 코드를 직접 선택해 복사해주세요.',
      });
    }
  }
    ...
    <div className="space-y-6" data-testid="admin-coupons-page">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">쿠폰을 생성하고 고객에게 발급합니다.</p>
        </div>
        <button
          type="button"
          className={`${buttonClass} inline-flex items-center gap-2`}
          disabled={!authHeaders || couponsQuery.isFetching}
      ...
      <div className="rounded-md border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
        할인과 최소 주문금액은 배달비를 제외한 상품 금액 기준입니다. 최소 주문금액은 매장 최소 주문금액 이상으로
        설정하세요. 비용을 예측하기 쉬운 정액 할인을 권장합니다.
      </div>
      ...
      <section className="grid items-start gap-6 lg:grid-cols-[360px_1fr]">
        <form onSubmit={submitCreate} className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          <fieldset disabled={!authHeaders || createMutation.isPending} className="space-y-4">
            ...
            <Field
              label={`할인값 (${form.type === 'PERCENTAGE' ? '%' : '원'}) *`}
              type="number"
              min={1}
            ...
            {/* 상한이 없으면 고액 주문에서 할인액이 무한정 커지므로 정률일 때는 필수로 받는다. */}
            {form.type === 'PERCENTAGE' && (
              <Field label="정률 할인 상한 (원) *" type="number" min={1} step={1} required {...bind('maxDiscountAmount')} />
            )}
            ...
            <p className="text-xs text-gray-500">
              코드가 있으면 고객이 직접 등록할 수 있습니다. 미입력 시 관리자 직접 발급 전용입니다.
            </p>
        ...
        <div className="min-w-0 space-y-4">
          ...
          {couponsQuery.isError ? (
            <div
              role="alert"
              data-testid="admin-coupons-fetch-error"
            ...
            couponsQuery.data.map((coupon) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <h3 className="break-words font-semibold text-gray-900">{coupon.name}</h3>
                  <span
                ...
                {coupon.description && (
                  <p className="break-words text-sm text-gray-500">{coupon.description}</p>
                )}

                <p className="font-semibold text-blue-700">{formatDiscount(coupon)}</p>

                <div className="space-y-1 text-sm text-gray-600">
                  <p>
                    최소 주문금액: {coupon.minOrderAmount ? `${formatNumber(coupon.minOrderAmount)}원` : '없음'}
                  </p>
                  {/* 한도는 발급 수 기준이므로 사용 수와 한 칸에 섞어 쓰지 않는다. */}
                  <p>
                    발급: {formatNumber(coupon.issuedCount)}장
                    {coupon.maxUses == null ? ' (한도 없음)' : ` / 한도 ${formatNumber(coupon.maxUses)}장`}
                  </p>
                  <p>사용: {formatNumber(coupon.usedCount)}장</p>
                  <p>유효기간: 발급일부터 {coupon.defaultExpiryDays}일</p>
                  <p>
                    생성일:{' '}
                    {new Date(coupon.createdAt).toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}
                  </p>
                </div>

                {coupon.code && (
                  <div className="flex items-center gap-2 text-sm">
                    <code className="break-all rounded bg-gray-100 px-2 py-1">{coupon.code}</code>
                    <button
                      type="button"
                      className={buttonClass}
                      aria-label={`${coupon.name} 프로모 코드 복사`}
                ...
                {issuingId === coupon.id ? (
                  <form className="border-t pt-4" onSubmit={(event) => submitIssue(event, coupon)}>
                    <fieldset
                      disabled={issueMutation.isPending || !authHeaders || !coupon.isActive}
                      className="space-y-3"
                    >
                      ...
                      />
                      <p className="text-xs text-gray-500">
                        고객의 사용자 ID를 입력하세요. 이메일이나 전화번호로는 발급되지 않습니다.
                      </p>
                      ...
                      <Field
                        ...
                        min={1}
                        step={1}
                        placeholder={`미입력 시 ${coupon.defaultExpiryDays}일`}
                        value={expiryDays}
                        onChange={(event) => setExpiryDays(event.target.value)}
                      />

                      <div className="flex gap-2">
                        <button type="submit" disabled={!userId.trim()} className={primaryClass}>
                          {issueMutation.isPending ? '발급 중…' : '발급 확인'}
                        </button>
                        <button type="button" className={buttonClass} onClick={() => setIssuingId(null)}>
                          취소
                        </button>
                      </div>
                    </fieldset>
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      className={buttonClass}
                      disabled={!coupon.isActive || !authHeaders || issueMutation.isPending}
                      onClick={() => openIssueForm(coupon.id)}
                    >
                      발급
                    </button>
                    <button
                      type="button"
                      className={buttonClass}
                      disabled={!authHeaders || activeMutation.isPending}
                      onClick={() =>
                        activeMutation.mutate({ couponId: coupon.id, isActive: !coupon.isActive })
                      }
                    >
