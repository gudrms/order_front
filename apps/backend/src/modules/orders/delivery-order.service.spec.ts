...
describe('DeliveryOrderService', () => {
    ...
    beforeEach(() => {
        ...
        service = new DeliveryOrderService(prisma, {} as any);
    });
    ...
    it('rejects cash delivery orders', async () => {
        ...
        await expect(service.createDeliveryOrder('store-1', {
            ...dto,
            payment: { ...dto.payment, method: 'CASH', paymentKey: 'CASH_ORDER_1' },
    ...
    it('rejects delivery orders without an authenticated user', async () => {
        ...
        await expect(service.createDeliveryOrder('store-1', { ...dto, userId: undefined }))
            .rejects.toBeInstanceOf(BadRequestException);
    ...
    it('lists delivery orders by authenticated user', async () => {
        ...
        const result = await service.getDeliveryOrders({ storeId: 'store-1', userId: 'user-1', page: 1 });
        ...
        expect(prisma.order.findMany).toHaveBeenCalledWith(expect.objectContaining({
            where: { type: 'DELIVERY', storeId: 'store-1', userId: 'user-1' },
        }));
    });
    ...
    it('returns a delivery order detail when the authenticated user matches', async () => {
        prisma.order = {
            findUnique: vi.fn().mockResolvedValue({ id: 'order-1', userId: 'user-1' }),
        };
    ...
    it('cancels a pending payment delivery order owned by the authenticated user', async () => {
        ...
        const result = await service.cancelDeliveryOrder('order-1', { userId: 'user-1', reason: 'wrong order' });
        ...
        expect(tx.payment.updateMany).toHaveBeenCalledWith(expect.objectContaining({
            where: { orderId: 'order-1', status: { in: ['READY', 'PENDING'] } },
    ...
    it('keeps delivery cancellation idempotent when the order is already cancelled', async () => {
        ...
        await expect(service.cancelDeliveryOrder('order-1', { userId: 'user-1' })).resolves.toEqual(cancelledOrder);

        expect(tx.payment.updateMany).not.toHaveBeenCalled();
        expect(tx.order.update).not.toHaveBeenCalled();
    });

    it('rejects customer cancellation for paid delivery orders', async () => {
        ...
        await expect(service.cancelDeliveryOrder('order-1', { userId: 'user-1' })).rejects.toBeInstanceOf(BadRequestException);
