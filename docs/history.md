...
| 관리자 회원가입 | 이메일 인증 유지 + `/auth/callback` | 무료 SMTP 한도가 문제였을 뿐, 인증 자체는 내부 관리자 도구에 필수. 커스텀 SMTP로 해결 가능 |
| 관리자 초대코드 | `/setup` 단계에서 입력, 로그인과 분리 | 매장 생성 시 자동 발급 → 1회 소모 구조가 이미 설계됨. 로그인 페이지에 중복 검증 불필요 |
