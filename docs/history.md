...
부분 환불은 초기 운영 범위에서 제외했다. 동시 부분 환불까지 안전하게 지원하려면 환불 잠금, 정산/고객 안내, 잔여 금액 UI가 함께 필요하므로 런칭 복잡도 대비 효율이 낮다. 관리자 UI는 전액 취소만 노출하고, 백엔드 환불 API는 `cancelAmount`가 포함된 부분 환불 요청을 `400`으로 거부하도록 정리했다.

---

## 13. 브랜드 메뉴·매장 데이터 구축 (2026-05-18)

브랜드 메뉴 관리 기능을 실제 운영 가능한 수준으로 보강했다. admin `브랜드 메뉴` 화면에 이미지 업로드(매장 메뉴와 같은 `MenuImageUpload` 컴포넌트 재사용, Supabase Storage `assets/brand-menu/` 경로), 인라인 수정 폼, 메뉴 정렬(`displayOrder`), 카테고리 숨김/노출 토글을 추가했다.

브랜드 메뉴의 `price`는 필수에서 선택값(`Int?`)으로 전환했다. 브랜드 메뉴는 홈페이지에 노출되는 대표 카탈로그이고 실제 판매 가격은 매장마다 다르므로, 가격 없이 이름·이미지만으로 등록할 수 있어야 한다는 판단이다. 스키마 마이그레이션과 함께 admin·홈페이지 표시 로직도 가격이 없을 때 가격 영역을 숨기도록 정리했다.

이후 타코몰리 메뉴판(.ai)에서 확인한 7개 대표 메뉴를 타코/부리또/케사디야/사이드/음료 5개 카테고리로 이미지와 함께 등록했다.

매장 데이터도 정리했다. 네이버 등록 정보 기준으로 7개 매장(김포·부천·부평·검단풍무·만수구월·루원시티·검단마전)의 주소·전화번호를 확정하고, 카카오 REST API로 좌표를 지오코딩해 `Store` 테이블에 등록했다. 기존 김포점은 연결된 메뉴 29개와 주문 1건을 보존하기 위해 삭제 대신 업데이트했다. 브랜드 사이트 `/store` 지도와 홈 매장 섹션은 `GET /stores`를 사용하므로 별도 작업 없이 자동 반영된다.

브랜드 사이트 `/brand` 페이지의 이모지 플레이스홀더는 실제 사진으로 교체했다. 추후 대표가 제공하는 사진으로 쉽게 바꿀 수 있도록 `public/brand/`의 고정 파일명을 사용했다. 창업 스토리 텍스트는 사실 기반으로 작성해야 하므로 대표 회신을 받은 뒤 교체하기로 했다.

이 과정에서 발견한 두 가지 문제도 함께 고쳤다. admin 사이드바의 `adminNavItems`에 동일 라우트를 가리키는 중복 메뉴('가맹 문의'·'창업 문의')가 있어 React key 중복 경고가 났고 '창업 문의'를 제거했다. e2e 빈 로그인 폼 테스트는 로그인 input의 HTML5 `required` 때문에 커스텀 에러 대신 브라우저 기본 검증이 작동하는 실제 동작과 어긋나 있어, `input:invalid` 검증으로 정정했다.
