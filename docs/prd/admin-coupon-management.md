...
- [ ] 총 발급 한도 보장: 직접 발급은 한도를 검사하지 않으며, 코드 등록은 발급 수 대신 실제 사용 수(`usedCount`)를 검사. 동시 발급까지 고려한 서버 보완 필요
- [ ] 중복 프로모 코드 생성·동일 고객 재발급 시 DB 오류를 명확한 메시지로 처리
- [ ] 활성 토글: 활성 여부 표시만 구현. 토글용 수정 API와 화면 연결은 이번 범위 제외
