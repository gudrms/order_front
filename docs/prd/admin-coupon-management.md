...
- [x] 실 ADMIN 계정·API로 쿠폰 생성 (2026-09-21, 운영): `POST /coupons` 201. 정액 3,000원·최소 15,000원·한도 2장·프로모 코드 저장 확인. 중복 코드 재생성 400, `PATCH :id/active` 200 및 비활성 시 발급 버튼 차단 확인. *정률 쿠폰은 상한 입력 필드 노출까지만 확인하고 실제 생성은 하지 않음*
- [x] 발급한 쿠폰이 배달앱 고객 화면(마이페이지 > 쿠폰)에 보이는지 (2026-09-21): 프로모 코드 등록 성공, 재등록 시 차단, 관리자 목록 `발급: 1장 / 한도 2장`으로 증가
- [x] 결제 화면에서 해당 쿠폰 적용 시 **상품 금액 기준**으로 할인되는지 (2026-09-21): 배달비는 그대로 두고 상품 금액에서만 3,000원 차감 확인
- [ ] 총 발급 한도 초과(3장째) 차단 — 고객 계정이 하나 더 필요해 미검증
- [ ] 관리자 직접 발급(userId 입력) 경로 — 관리자 화면에 고객 목록이 없어 user ID를 얻지 못해 미검증
- [x] 모의 조회 API 500일 때 빈 목록 대신 에러 배너 표시 및 재시도 복구
- [ ] ESLint 검증: 관리자 앱의 ESLint 설정 파일 부재로 실행 불가

`e2e/admin/coupons.spec.ts`의 모의 API E2E 5개 통과(활성 토글 포함). 실 운영 연동은 2026-09-21에 별도로 확인했습니다(위 체크 참고). 검증에 쓴 쿠폰 `[검증용] 삭제예정 정액3000`(코드 `VERIFY0921`)은 삭제 API가 없어 비활성 상태로 남겨 두었습니다.
