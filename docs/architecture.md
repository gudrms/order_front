...
POST   /pos/catalogs/sync     POS 메뉴 동기화 (API 키 인증)
...
4. pgmq order.paid 이벤트 → 주문 확정, FCM 푸시 발송
```
...
admin 앱만 Supabase Realtime WebSocket을 사용.

| 훅 | 채널 | 구독 테이블 | 이벤트 |
|---|---|---|---|
| `useRealtimeOrders` | `orders:${storeId}` | orders | INSERT, UPDATE, DELETE |
| `useStaffCalls` | `staff-calls:${storeId}` | StaffCall | INSERT |

table-order는 현재 5초 폴링 방식 (Realtime 전환 예정 — checkList.md P2 참고).
...
| 인증 없음 | `GET /stores`, `GET /stores/:id/menus`, `/stores/:id/orders/*` (table-order용) | - |
